...
for metric in metrics:
    ...
    if metric == "lp":
        ...
        outlier = Outlier(
            model_path=model_path, o_scores_path=o_scores_path, metric=metric
        )
    ...
    most_normal_ids, most_oulying_ids = outlier.top_reconstructions(
        O=training_data, n_top_spectra=n_top_spectra
    )
    ###############################################################################
    ...
    for idx, spec_idx in enumerate(most_oulying_ids):
        ...
        sdss_name, sdss_name_path = outlier.metadata(
            spec_idx=spec_idx, training_data_files=sdss_spec_paths
        )
