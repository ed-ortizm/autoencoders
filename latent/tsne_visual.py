"""TSNE 2D embedding visualization"""
...
config_file_name = "tsne_visual.ini"
...
for model_idx, latent_directory in enumerate(latent_directories):
    ...
    print(f"model {models_ids[model_idx]}: TSNE", end="\n")

    # load tsne embedding
    for metric in metrics:

        embedding = np.load(f"{latent_directory}/tsne_{metric}_{bin_id}.npy")
    ...
    for hue in parameters_of_plot["hues"]:
        ...
        for metric in metrics:
            ...
            print(f"TSNE metric: {metric}. Hue: {hue}", end="\r")
            ...
            save_to = f"{latent_directory}/tsne"
            manage_files.check_directory(save_to, exit_program=False)

            fig.savefig(
                f"{save_to}/"
                f"tsne_{metric}_{hue}.{parameters_of_plot['format']}"
            )
